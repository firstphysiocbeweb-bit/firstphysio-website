...
(function() {
    ...
    function triggerPractoWidget() {
        ...
        // Try to find and click the Practo widget
        const widget = document.querySelector('practo\\:abs_widget');
        if (widget) {
            logPracto('Found Practo widget element');
            ...
            // Method 2: Look for clickable element inside widget
            const clickableElement = widget.querySelector('button, a, [role="button"], [onclick]');
            if (clickableElement) {
                logPracto('Found clickable element inside widget');
                clickableElement.click();
                return;
            }
        }

        // Method 3: Check if PractoWidget API is available
        if (window.PractoWidget && typeof window.PractoWidget.open === 'function') {
            logPracto('Using PractoWidget.open() API');
            window.PractoWidget.open({
                widgetId: PRACTO_CONFIG.widgetId
            });
            return;
        }

        // Fallback: Redirect to Practo profile
        logPracto('Using fallback redirect to Practo profile', 'warn');
